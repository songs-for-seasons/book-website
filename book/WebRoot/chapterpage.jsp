	...
	<script tyoe="text/javascript">
		function showdiv(name1,name2,rid,time)
		{
			...
			else
			{
					...
					document.getElementById("recomment").setAttribute("rid", rid);
					document.getElementById("recomment").setAttribute("time", time);
			}
  	...
  	<div id="showcomment">
  		<table id="rlist">
  			<% 	RemarkDAO r=new RemarkDAO();
				...
				ArrayList<Remark> list=r.select(bID);
			 ...
			 %>
				<tr>
					...
					<td><a href="#" onclick="showdiv('<%=userName%>','<%=wName%>','<%=name%>','<%=list.get(i).getTime()%>')">回复</a></td>
				</tr>
				<tr>
					<td>评论内容</td>
